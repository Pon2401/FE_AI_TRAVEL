    ...
    <section class="hero-section py-5 position-relative">
      ...
      <div class="container text-center">
        ...
        <p class="page-subtitle mb-4 text-center">
          <i class="fas fa-star me-2"></i>Khám phá những địa điểm check-in đẹp nhất: Cầu Vàng, Cầu Rồng, Biển Mỹ Khê,
          Phố cổ Hội An... và nhận đề xuất từ AI sao cho phù hợp ngân sách của bạn.
        </p>
        <div class="search-bar d-flex justify-content-center mb-3">
          ...
          <button class="btn btn-outline-success ms-2" v-if="tempSearchQuery.length >= 2" @click="searchGoogle"
            :disabled="loadingSerp"><span v-if="loadingSerp" class="spinner-border spinner-border-sm"></span><i v-else
              ...
              <div class="serp-item d-flex align-items-start gap-3 p-3 rounded-4 transition-all">
                ...
                <div class="serp-item-content flex-grow-1">
                  ...
                  <div class="d-flex align-items-center gap-2 mb-2">
                    <span class="badge bg-light text-dark border"><i class="bi bi-geo-alt-fill text-danger me-1"></i>{{
                      res.vi_do }}, {{ res.kinh_do }}</span>
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const res = await fetch(`${BASE}/dia-diems/check-in`, { headers });
    ...
    async toggleFavorite(place) {
      ...
      try {
        ...
        if (json.status) {
          ...
          this.$toast.success(json.is_favorite ? `Đã thêm "${place.ten_dia_diem}" vào yêu thích! ❤️` : `Đã xoá "${place.ten_dia_diem}" khỏi yêu thích`);
        } else {
